...
def sim_population_frequencies(init_freqs, fitnesses, gens=4, N=1e12, D_lst=[3*10**5]*5, num_populations=5):
    ...
    for i in range(num_populations):
        ...
        reads, freqs = sim.double_poisson(pop.freqs(), D_lst[i], D_lst[i]) #double poisson sampling
        population_reads[i] = reads
        population_frequencies[i] = freqs
...
def generate_simulated_ordered_fitnesses(init_reads, D0, population_reads, D_tot, dt=4):
    ...
    f0, f1 = shared.maxmin_freqs(init_freqs, D0, mean_pop_freqs, D_tot)
    sim_fitnesses = shared.calc_lfc_array(f0, f1, dt)
...
def generate_sim_drift_estimates(bac, mice, splits, permutations, N, tau=1/10, min_group_size=50, min_freq_ratio=0.7, bootstrap=1000):
    ...
    if 'generate simulated data' != '':

        fitnesses, sorted_indices = generate_empirical_ordered_fitnesses(bac, mice, day)

        num_replicates = 9
        mice_freqs, mice_reads = sim_population_frequencies(init_freqs, fitnesses * tau, gens=int(4 / tau),
                                                            N=Ntau / tau,
                                                            num_populations=num_replicates, D_lst=D_lst)
        inferred_fitnesses, sorted_indices = generate_simulated_ordered_fitnesses(init_freqs[mask], D0,
                                                                                  mice_reads[:, mask], D_lst.sum(),
                                                                                  4)
...
def make_sfig(fig, ax, Ntau_drift_dict):
    for x, Ntau in enumerate(Ntau_drift_dict.keys()):
        drift_dict = Ntau_drift_dict[Ntau]
        slopes = []
        ...
        ax.scatter([Ntau] * outliers.sum(), Ntaus[outliers], color=KELLY_COLORS[0], marker='.', s=5)
        ax.errorbar([Ntau], [median], yerr=[[median - iqr_25], [iqr_75 - median]], color=KELLY_COLORS[0], mec='black',
                    marker='o')
    return
...
if __name__ == '__main__':
    ...
    N_lst = [10**6, 10**7, 10**8, 10**9, 10**10]
    tau = 1/10
    Ntau_drift_dict = {}
    for N in N_lst:
        Ntau = int(N*tau)
        print(f'> Inferring {bac} Ntau={Ntau}            ')

        drift_dict = generate_sim_drift_estimates(bac, mice, splits,
                                          rnd.permutation(permutations[:])[:n_permutations], N, tau=tau, bootstrap=n_bootstrap)
        Ntau_drift_dict[Ntau] = drift_dict

    fig, ax = plt.subplots(figsize=(3, 2))
    make_sfig(fig, ax, Ntau_drift_dict)
    format_sfig(ax)

    fig.savefig(f'{plot_dir}/sfig17A_drift_simBWH2_inference.pdf')
